...
class ExecuteIndirect :public D3D12AppBase, public D3D12Base
{
	...
	static const UINT triangleResourceCount = triangleCount*cnt_r_DefaultFrameCount;
	...
	ComPtr<ID3D12CommandAllocator> computeCommandAllocators[cnt_r_DefaultFrameCount];
	ComPtr<ID3D12RootSignature> computeRootSignature;
	ComPtr<ID3D12PipelineState> computeState;

	ComPtr<ID3D12Fence> fence;
	ComPtr<ID3D12Fence> computeFence;
	UINT64 fenceValues[cnt_r_DefaultFrameCount];
	HANDLE fenceEvent;
	...
	ComPtr<ID3D12Resource> processedCommandBuffers[cnt_r_DefaultFrameCount];
