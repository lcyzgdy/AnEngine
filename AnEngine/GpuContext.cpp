...
namespace AnEngine::RenderCore
{
	...
	void GpuContext::AttachSwapChain(const Microsoft::WRL::ComPtr<IDXGISwapChain4>& swapChain, const uint32_t bufferCount)
	{
		...
		for (int i = 0; i < m_bufferCount; i++)
		{
			...
			var displayPlane = new Resource::ColorBuffer(L"Primary SwapChain Buffer", displayPlane.Detach(),
				Heap::DescriptorHeapAllocator::GetInstance()->Allocate(D3D12_DESCRIPTOR_HEAP_TYPE_RTV));
		}
	}
}